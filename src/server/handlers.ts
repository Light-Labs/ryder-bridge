...
export default function(): ClientEventHandlers {
    return {
        serial_open: async function (payload: RyderSerialConfig, callback: (res: Response<string>) => void) {
            ...
            new Promise<string>((resolve, reject) => {
                ryder_serial.on("failed", (error: Error) => {
                    reject(`Could not connect to the Ryder on the specified port. Wrong port or it is currently in use. The error was: ${error}`)
                    return;
                });
